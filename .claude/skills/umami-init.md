...
**Last synced:** 2026-07-03 from §0.7b (step 4 now names the on-demand skills — `umami-auto-review` / `umami-pipeline-audit` / `umami-drift-audit` install only when discovery shows their protocol applies, closing the README-promises-five-commands vs. init-installs-two gap; previously: §0.9 adoption-ledger seeding + default-deny adoption on init 2026-06-18; auto-chain into `/umami-audit` on successful apply 2026-05-22). Re-derive this skill from the current §0.7b when this date is >3 months old or when an audit reports structural drift.
...
1. **Detect current state.** Grep instruction file(s) (`CLAUDE.md`, `AGENTS.md`, `.cursorrules`, `.cursor/rules/`, etc.) for umami raw-fetch URL references (`raw.githubusercontent.com/…/umami`) — repo-page links or prose mentions of umami don't count as an installation. Catalog which extensions (if any) are loaded. Determine starting state: **none** / **partial** / **complete**.
...
3. **Derive the recommended set** of URLs via §0.5's "if the project has..." mapping. The URL block always includes the **landing document** (`umami.md` at the repo root). Core companion files (`core/umami-quality.md`, `core/umami-runtime.md`, `core/umami-process.md`, `core/umami-agents.md`; `core/umami-anti-patterns.md` is fetched during onboarding/audits rather than listed per-tier) and domain extensions (`ext/umami-{name}.md`, with shared+variant clusters under `ext/{domain}/`) are added based on the tier the project is targeting and which concerns apply. For Tier 1 / first-pass adoption, the URL block may legitimately list only the landing plus relevant domain extensions — companions can be added when the project escalates to Tier 2.
...
- **Always fetch the landing document fresh** from the canonical URL (`https://raw.githubusercontent.com/goodcol-dennis/umami/refs/heads/main/umami.md`; use `refs/heads/develop` only for bleeding-edge, not-yet-released practices). Never cache locally. Skill files don't count — they're invocation aids, not copies. If the project pins a release tag (`refs/tags/vX.Y`), fetch that tag and preserve the pin in the URL block init writes.
...
- **Detect legacy v2.1 paths and offer migration.** If `CLAUDE.md` references pre-v3 paths (root-level `umami-X.md`, `cms/`, `desktop/`), surface this in the proposed diff. Offer to update the URLs to their v3 paths (`core/`, `ext/`, `ext/cms/`, `ext/desktop/`). **As of v3.1 legacy paths 404** — the v3.0 deprecation stubs were removed; an adopter still on legacy paths is currently fetching dead URLs. Flag the migration as a hard priority, not a recommendation.
