...
6. **Decide dispositions** per gate — Keep (still catches things and fits the team's needs) / Move local (catches issues that should be caught pre-push) / Demote to nightly (catches drift, not per-commit issues) / Remove (no longer earns its cost or doesn't fit needs) / Add (missing for the current risk profile). **Each disposition cites Impact and Effort** per the §0.7 audit-recommendation format: Impact names the specific improvement (cycle-time reduction, gate-mismatch closed, missing-coverage filled); Effort uses the §4 cost-profile scheme. Flag **Quick win** when Impact is concrete and Effort is Hours.
